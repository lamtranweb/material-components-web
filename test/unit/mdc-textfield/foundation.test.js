...
test('#init floats label if the input contains a value', () => {
  ...
  td.when(mockAdapter.getLabelFoundation()).thenReturn(label);
  foundation.init();
  td.verify(mockAdapter.getLabelFoundation().floatLabel());
});
...
test('#init does not float label if the input does not contain a value', () => {
  ...
  td.when(mockAdapter.getLabelFoundation()).thenReturn(label);
  foundation.init();
  td.verify(mockAdapter.getLabelFoundation().floatLabel(), {times: 0});
});
...
test('on input focuses and floats label if input event occurs without any other events', () => {
  const {foundation, mockAdapter} = setupTest();
  let input;
...
test('on blur floats label when no input value present', () => {
  const {mockAdapter, blur} = setupBlurTest();
  const label = td.object({
    floatLabel: () => {},
    deactivateFocus: () => {},
